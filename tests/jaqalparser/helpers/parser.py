...
class ParserTesterMixin:
    ...
    def simplify_tree(cls, tree):
        ...
        if isinstance(tree, Tree):
            return {
                "type": tree.data,
                "children": [cls.simplify_tree(child) for child in tree.children],
            }
    ...
    def make_array_declaration(cls, name, length):
        return {
            ...
            "children": [cls.make_identifier(name), cls.make_integer(length)],
        }
    ...
    def make_qualified_identifier(cls, *names):
        return {
            ...
            "children": [cls.make_identifier(name) for name in names],
        }
    ...
    def make_let_identifier(cls, name):
        return {
            "type": "let_identifier",
            "children": [cls.parse_qualified_identifier(name)],
        }

    @classmethod
    def make_let_or_map_identifier(cls, name):
        return {
            "type": "let_or_map_identifier",
            "children": [cls.parse_qualified_identifier(name)],
        }
    ...
    def make_map_statement(cls, target, source):
        return {
            "type": "map_statement",
            "children": [cls.make_identifier(target), source],
        }

    @classmethod
    def make_array_slice(cls, name, slice_start, slice_stop, slice_step):
        slice_start_children = (
            [cls.make_signed_integer(slice_start)] if slice_start is not None else []
        )
        slice_stop_children = (
            [cls.make_signed_integer(slice_stop)] if slice_stop is not None else []
        )
        slice_step_children = (
            [cls.make_signed_integer(slice_step)] if slice_step is not None else []
        )
        slices = [
    ...
    def make_array_element(cls, name, index):
        return {
            "type": "array_element",
            "children": [
                cls.parse_qualified_identifier(name),
                cls.make_array_index(index),
            ],
        }
    ...
    def make_let_statement(cls, name, value):
        return {
            "type": "let_statement",
            "children": [cls.make_identifier(name), cls.make_signed_number(value)],
        }
    ...
    def make_from_clause(cls, *module_names):
        return {
            "type": "from_clause",
            "children": [cls.make_qualified_identifier(*module_names)],
        }
    ...
    def make_gate_statement(cls, name, *args):
        ...
        return {
            "type": "gate_statement",
            "children": [cls.parse_qualified_identifier(name)] + arg_children,
        }
    ...
    def make_macro_statement(cls, name, *args):
        ...
        macro_header = {
            "type": "macro_header",
            "children": [cls.make_identifier(name)]
            + [cls.make_identifier(arg) for arg in args],
        }
        macro_gate_block = {"type": "macro_gate_block", "children": [gate_block]}
        return {
            "type": "macro_definition",
            "children": [macro_header, macro_gate_block],
        }
