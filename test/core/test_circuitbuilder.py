...
class BuildTester(unittest.TestCase):
    ...
    def test_build_map_slice(self):
        ...
        sexpr = ('circuit', ('register', rname, rsize), ('map', mname, rname, mstart, mstop, mstep))
        circuit = build(sexpr)
        act_value = circuit.registers[mname]
        self.assertEqual(exp_value, act_value)
    ...
    def test_build_macro_definition(self):
        ...
        exp_value = core.Macro('foo', parameters=[Parameter('a', None), Parameter('b', None)])
        exp_value.body.statements.append(core.GateStatement(gate_def, parameters={'x': Parameter('a', None)}))
        exp_value.body.statements.append(core.GateStatement(gate_def, parameters={'x': Parameter('b', None)}))
        self.assertEqual(exp_value, act_value)
    ...
    def test_build_native_gate(self):
        name = randomize.random_identifier()
        parameters = [
            Parameter('a', core.INT_TYPE),
            Parameter('b', core.FLOAT_TYPE)
        ]
    ...
    def test_build_map_with_let(self):
        sexpr = ('circuit', ('let', 'a', 1), ('register', 'r', 10), ('map', 'q', 'r', 'a'))
        reg = Register('r', 10)
        const = Constant('a', 1)
    ...
    def test_build_map_slice_with_let(self):
        sexpr = ('circuit', ('let', 'a', 0), ('let', 'b', 3), ('let', 'c', 2), ('register', 'r', 3),
                 ('map', 'q', 'r', 'a', 'b', 'c'))
        reg = Register('r', 3)
        const_a = Constant('a', 0)
        const_b = Constant('b', 3)
        const_c = Constant('c', 2)
        exp_value = Register('q', alias_from=reg, alias_slice=slice(const_a, const_b, const_c))
        circuit = build(sexpr)
        act_value = circuit.registers['q']
        self.assertEqual(exp_value, act_value)
